...
import { apiClient, apiGet, apiPost, apiPatch, apiDelete } from '@/app/lib/utils/api-client'
...
export function useAdmin() {
  ...
  const fetchRoles = useCallback(async () => {
    ...
    try {
      ...
      setLoading(true)
      setError(null)
      const result = await apiGet('/api/admin/roles')
  ...
  const fetchAuditLogs = useCallback(async (filters: any = {}) => {
    ...
    try {
      ...
      const result = await apiGet(`/api/admin/audit?${params}`)
  ...
  const updateUser = async (userId: string, userData: {
    ...
    try {
      setLoading(true)
      const result = await apiPatch('/api/admin/users', { userId, ...userData })
  ...
  const deactivateUser = async (userId: string) => {
    try {
      setLoading(true)
      const result = await apiDelete(`/api/admin/users?userId=${userId}`)
  ...
  const createRole = async (roleData: {
    ...
    try {
      setLoading(true)
      const result = await apiPost('/api/admin/roles', roleData)
  ...
  const updateRole = async (roleId: string, roleData: {
    ...
    try {
      setLoading(true)
      const result = await apiPatch('/api/admin/roles', { roleId, ...roleData })
