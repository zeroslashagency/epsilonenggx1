...
export class BackendIntegrationService {
  ...
  private async initializeSchedulingEngine(): Promise<void> {
    const masterData = this.services.masterData && this.services.masterData.length > 0
      ? this.services.masterData
      : this.getFallbackMasterData()

    // Deterministic fallback scheduler used when legacy browser modules are unavailable.
    this.services.schedulingEngine = new DeterministicSchedulingEngine(masterData)
  }
  ...
  async runSchedule(orders: any[], settings: any): Promise<any> {
    ...
    throw new Error(
      'No scheduling engine available. Enable legacy modules (NEXT_PUBLIC_ENABLE_LEGACY_SCHEDULING_MODULES=true) or configure deterministic scheduler backend.'
    )
  }
