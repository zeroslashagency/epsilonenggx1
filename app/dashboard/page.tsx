...
function DashboardPageContent() {
  ...
  // ⚡ PERFORMANCE: Parallelized API calls (was 7 sequential, now parallel)
  const fetchAttendanceTrend = useCallback(async () => {
    try {
      const today = new Date()

      // Build array of date strings for last 7 days
      const datePromises = Array.from({ length: 7 }, (_, i) => {
        const date = new Date(today)
        date.setDate(date.getDate() - (6 - i)) // Start from oldest
        const dateStr = date.toISOString().split('T')[0]
        return apiGet(`/api/get-attendance?fromDate=${dateStr}&toDate=${dateStr}`)
      })

      // Fetch all 7 days in parallel (7× faster)
      const responses = await Promise.all(datePromises)

      const trends: AttendanceTrend[] = responses.map((response, i) => {
        const date = new Date(today)
        date.setDate(date.getDate() - (6 - i))
        const present = response.success ? (response.data?.employeeStatus?.length || 0) : 0
      ...
      // Silent fail - trends are non-critical
    }
                ...
                <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
                  ...
                  <div className="relative hidden md:block">
                    ...
                    <Input
                      ...
                      className="pl-9 w-48 lg:w-64 bg-white/10 border-white/20 text-white placeholder:text-white/60"
                    />
                  </div>
              ...
              {canViewMetrics && (
                <>
                  ...
                  <div className="space-y-4 min-w-0">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-6 shadow-lg border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-all duration-300">
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-0 mb-4 sm:mb-6">
                        ...
                        <div className="inline-flex items-center bg-gray-100 dark:bg-gray-700 rounded-full p-1 w-full sm:w-auto">
                          ...
                          <button className="flex-1 sm:flex-none px-3 sm:px-6 py-1.5 sm:py-2 rounded-full text-gray-600 dark:text-gray-400 font-medium text-xs sm:text-sm hover:text-gray-900 dark:hover:text-white transition-all">
                            Weekly
                          </button>
                          <button className="flex-1 sm:flex-none px-3 sm:px-6 py-1.5 sm:py-2 rounded-full text-gray-600 dark:text-gray-400 font-medium text-xs sm:text-sm hover:text-gray-900 dark:hover:text-white transition-all">
                            Monthly
