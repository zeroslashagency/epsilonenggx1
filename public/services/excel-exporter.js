...
class ExcelExporter {
        ...
        if (scheduleData.rows && scheduleData.rows.length > 0) {
            ...
            scheduleData.rows.forEach(row => {
                if (!uniqueParts[row.PartNumber]) {
                    // Get all operation sequences for this part
                    const partRows = scheduleData.rows.filter(r => r.PartNumber === row.PartNumber);
                    const operationSeqs = partRows.map(r => r.OperationSeq).sort((a, b) => a - b);
                    const operationSeqRange = operationSeqs.length > 1 ? 
                        `${operationSeqs[0]}-${operationSeqs[operationSeqs.length - 1]}` : 
                        operationSeqs[0].toString();
                    
                    uniqueParts[row.PartNumber] = {
                        PartNumber: row.PartNumber || '',
                        OperationSeq: operationSeqRange,
                        Order_Quantity: row.Order_Quantity || 0,
                        priority: (row.Priority || 'normal').toLowerCase(),
                        dueDate: row.DueDate || '',
                        breakdownMachine: '', // Empty - no specific breakdown machine in input
                        Breakdown_Machines_date_time: this.getBreakdownDateTime(),
     ...
     * @param {string} machine - Machine name (optional)
     * @returns {string} Breakdown date time
     */
    getBreakdownDateTime(machine = null) {
        // Get breakdown data from global application settings
        if (typeof window !== 'undefined' && window.breakdowns && Array.isArray(window.breakdowns) && window.breakdowns.length > 0) {
            if (machine) {
                const breakdown = window.breakdowns.find(b => b.machines && b.machines.includes(machine));
                if (breakdown && breakdown.startDateTime && breakdown.endDateTime) {
                    return `${breakdown.startDateTime} - ${breakdown.endDateTime}`;
                }
            } else {
                // Return all breakdowns if no specific machine
                const validBreakdowns = window.breakdowns.filter(b => b.startDateTime && b.endDateTime);
                if (validBreakdowns.length > 0) {
                    return validBreakdowns.map(b => `${b.startDateTime} - ${b.endDateTime}`).join('; ');
                }
            }
        }
        return 'No breakdowns configured';
    }
    ...
    getHolidayInfo() {
        ...
        if (typeof window !== 'undefined' && window.holidays && Array.isArray(window.holidays) && window.holidays.length > 0) {
            const validHolidays = window.holidays.filter(h => h.startDateTime && h.endDateTime);
            if (validHolidays.length === 0) return 'No holidays configured';
            return validHolidays.map(h => `${h.startDateTime} - ${h.endDateTime} (${h.reason || 'Holiday'})`).join('; ');
        }
