...
import { Priority, Attachment, User } from '../types';
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-200">
                ...
                {/* Header */}
                <div className={`p-4 flex justify-between items-center ${type === 'GOOD' ? 'bg-green-600' : 'bg-red-600'} text-white shrink-0`}>
                ...
                <div className="flex-1 overflow-y-auto">
                    {step === 'SELECT_EMPLOYEE' ? (
                        <div className="p-6">
                            <h4 className="text-lg font-semibold text-slate-800 mb-4">Select Employee</h4>
                            {loadingEmployees ? (
                                ...
                                <div className="space-y-2">
                                    {employees.map(emp => (
                                        <button
                                            ...
                                            className="w-full flex items-center justify-between p-4 rounded-xl border border-slate-200 hover:border-indigo-500 hover:bg-indigo-50 transition-all group text-left"
                                        >
                                            <div>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div>
                                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">What happened?</label>
                                <input
                                    required
                                    type="text"
                            ...
                            {/* Evidence Section */}
                            <div>
                                <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Evidence</label>

                                <div className="space-y-2 mb-3">
                                    {attachments.map(att => (
                                        <div key={att.id} className="relative group border border-slate-200 rounded-lg p-2 bg-slate-50 flex items-center gap-3">
                                            {att.type === 'image' ? (
                                                <div className="h-12 w-12 shrink-0 bg-slate-200 rounded overflow-hidden">
                                                    <img src={att.url} alt="thumb" className="h-full w-full object-cover" />
                                                </div>
                                            ) : att.type === 'audio' ? (
                                                <div className="flex items-center gap-2 w-full">
                                                    <div className="h-10 w-10 shrink-0 bg-indigo-100 rounded-full flex items-center justify-center text-indigo-600">
                                                        <Mic size={20} />
                                                    </div>
                                                ...
                                                <div className="h-12 w-12 shrink-0 bg-slate-200 rounded flex items-center justify-center">
                                            ...
                                            <button
                                                ...
                                                className="ml-auto p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-full transition"
                                ...
                                <div className="flex gap-2">
                                    <label className="cursor-pointer flex items-center gap-2 px-3 py-2 bg-slate-100 hover:bg-slate-200 rounded-lg text-slate-700 text-sm transition">
                                        <Camera size={16} />
                                        <span>Photo</span>
                ...
                <div className="p-4 border-t bg-slate-50 flex justify-end gap-3 shrink-0">
                    {step === 'FILL_FORM' && (
                        <button
                            ...
                            className="px-4 py-2 text-slate-600 font-medium hover:text-slate-800 mr-auto"
                        >
                            Back
                        </button>
                    )}
