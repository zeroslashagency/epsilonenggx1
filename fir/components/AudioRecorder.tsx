...
export const AudioRecorder: React.FC<AudioRecorderProps> = ({ onSave }) => {
  ...
  return (
    <div className={`flex items-center gap-2 p-2 rounded-full transition-colors ${isRecording ? 'bg-red-50 border border-red-200' : 'bg-gray-100'}`}>
      {isRecording ? (
        <>
          <span className="text-red-600 animate-pulse text-xs font-bold px-2">REC {formatTime(duration)}</span>
          <button
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 text-gray-700 hover:text-gray-900"
          type="button"
        >
          <Mic size={18} />
