# Production Scheduler Piece-Level Pipeline PRD

Date: 2026-02-23
Owner: Scheduling QA/Verification
Input dataset: `/Users/xoxo/Downloads/import_input.xlsx`
Target: 120 runnable cases, 500 atomic checks

## 1) Objective

Build and execute a piece-level validation program for the Production Scheduler import pipeline, from small batches to highly complex multi-constraint scenarios, with explicit error-handling verification and audit-ready reporting.

## 2) In Scope

- Excel import parsing and alias handling in scheduler import flow.
- Operation detail normalization and fallback behavior.
- Personnel block parsing and assignment behavior.
- Deterministic scheduling correctness and piece timeline integrity.
- Conflict detection parity with Python conflict suite taxonomy.
- Error/warning quality (deterministic codes, actionable message text).
- Performance and reproducibility checks.

## 3) Out of Scope

- UI redesign and style changes.
- Production DB schema changes.
- New scheduling algorithm design (unless required by defect remediation).

## 4) Architecture Under Test

- Import pipeline: `app/(app)/scheduler/page.tsx`
- Scheduler engine: `app/lib/features/scheduling/deterministic-scheduling-engine.ts`
- Piece verifier module: `app/lib/features/scheduling/piece-flow-verifier.ts`
- Python verifier: `scripts/piece_level_verifier.py`
- Conflict suite: `scripts/piece_conflict_suite.py`

## 5) Multi-Agent Execution Model

- Agent A: Import contract and schema/error checks.
- Agent B: Scheduling rules, machine/person overlap checks.
- Agent C: Piece timeline and precedence verification.
- Agent D: Performance, stress, stability, reproducibility.
- Agent E: Report synthesis, triage and release recommendation.

## 6) Complexity Ladder

- L1 Tiny: 1 part, 1-2 operations, quantity 1-5.
- L2 Small: 3-5 parts, mixed handle modes.
- L3 Medium: 10+ parts, due dates and setup windows.
- L4 Large: 25+ parts, 200+ piece events.
- L5 Complex: 33+ parts, holidays + breakdowns + machine availability + constrained personnel.

## 7) 120 Runnable Case Plan

- C001-C012: Import happy path and alias coverage.
- C013-C024: Required-field failures and schema corruption.
- C025-C036: Numeric and datetime/window validation failures.
- C037-C048: Personnel section parsing edge cases.
- C049-C060: Fixed machine, eligible machine, handle mode interactions.
- C061-C072: Setup/run window, holiday, breakdown, availability constraints.
- C073-C084: Piece precedence and operation chain integrity.
- C085-C096: Quantity conservation and split batch behavior.
- C097-C108: Conflict taxonomy parity and code mapping checks.
- C109-C114: Large-batch performance and repeatability.
- C115-C120: End-to-end audit artifact completeness and release gate checks.

## 8) Exit Criteria

- No Sev-0 or Sev-1 defects open.
- 100% pass on critical checks.
- > = 98% overall check pass rate.
- Deterministic rerun stability within tolerance.
- Full evidence package generated for all failures.

## 9) Atomic Checklist (500 Tasks)
